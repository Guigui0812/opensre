...
class HelmConfig(StrictConfigModel):
    ...
    def is_configured(self) -> bool:
        # HELM_KUBECONFIG is optional (falls back to ~/.kube/config)
        # Only HELM_PATH is required for the integration to be usable
...
def validate_helm_config(config: HelmConfig) -> HelmValidationResult:
    ...
    # helm_path always has a default value, so the empty check is unreachable
...
def check_diff(
    ...
    This compares the live resources against the release's stored manifest.
    Uses 'helm diff release' which does not require a chart reference.
